    ...
    def filter_queryset(self, request, queryset, view):
        ...
        if view.lookup_field not in view.kwargs:
            if not self.action_routing:
                ...
                method_name = "filter_{action}_queryset".format(
                    action=view.action)
...
class DRYPermissions(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        if hasattr(view, 'action'):
            ...
            action_method_name = "has_{action}_permission".format(
                action=action)
            # If the specific action permission exists then use it, otherwise
            # use general.
    ...
    def has_object_permission(self, request, view, obj):
        ...
        if hasattr(view, 'action'):
            action = self._get_action(view.action)
            action_method_name = "has_object_{action}_permission".format(
                action=action)
            # If the specific action permission exists then use it, otherwise
            # use general.
...
class DRYPermissionsField(fields.Field):
    ...
    def to_representation(self, value):
        ...
        for action, method_names in self.action_method_map.items():
            # If using global permissions and the global method exists for this
            # action.
            if not self.object_only and method_names.get('global', None) is not None:
                results[action] = getattr(self.parent.Meta.model, method_names[
                                          'global'])(self.context['request'])
